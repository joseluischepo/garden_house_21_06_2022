...
$titulo = $precio = $foto = $sup = $descri = $vr = $Idtipo = $pais = $edo = $loca = $col = $calle = $cruza = $num = "";
$titulo_err = $precio_err = $foto_err = $sup_err = $descri_err = $vr_err = $Idtipo_err = $pais_err = $edo_err = $loca_err = $col_err = $calle_err = $cruza_err = $num_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
   ...
   // Validate pais
    ...
    if(empty($titulo_err) && empty($precio_err) && copy($ruta, $foto) && empty($sup_err) && empty($descri_err) && empty($vr_err) && empty($Idtipo_err) && empty($pais_err) && empty($edo_err) && empty($loca_err) && empty($col_err) && empty($calle_err) && empty($cruza_err) && empty($num_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO casa2 (titulo,precio,foto,sup,descri,vr,Idtipo,pais,edo,loca,col,calle,cruza,num) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssssss", $param_titulo, $param_precio,$param_foto,$param_sup,$param_descri,$param_vr,$param_Idtipo,$param_pais,$param_edo,$param_loca,$param_col,$param_calle,$param_cruza,$param_num);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: createCasa2.php"); //pendiente checar esto
                exit();
            } else{
          ...
          <div class="col-md-12 mb-3 mb-md-0">
                ...
                <br>
                  Nombre <input type="file" name="img" ><br><br>
                        ...
                        <div class="form-group <?php echo (!empty($sup_err)) ? 'has-error' : ''; ?>">
                            ...
                            <input type="text" name="sup" class="form-control" value="<?php echo $sup; ?>">
                            <span class="help-block"><?php echo $sup_err;?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($descri_err)) ? 'has-error' : ''; ?>">
                            <label>Descripcion</label>
                            <textarea name="descri" class="form-control"><?php echo $descri; ?></textarea>
            ...
            <div class="col-md-3">
               	<div class="form-group <?php echo (!empty($loca_err)) ? 'has-error' : ''; ?>"> 			 		
                     ...
                     <label class="text-black" for="lname">Municipio</label>
            ...
            <div class="col-md-3">
               	<div class="form-group <?php echo (!empty($col_err)) ? 'has-error' : ''; ?>"> 			 		
                     ...
                     <label class="text-black" for="lname">Colonia</label>
