...
if(isset($_POST["idc"]) && !empty($_POST["idc"])){
   ...
   //Validar celular 1 (cel1)
    ...
    if(empty($input_contra)){
        $contra_err = "Por favor ingrese el nombre del empleado.";
    } elseif(!filter_var($input_contra, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
        $contra_err = "Por favor ingrese un nombre válido.";
    } else{
        $contra = $input_contra;
    }
   ...
   //Validar numero de casa
    ...
    if(empty($name_err) && empty($ape1_err) && empty($ape2_err) && empty($correo_err) && empty($cel1_err) && empty($cel2_err) && empty($tel_err) && empty($contra_err) && empty($pais_err) && empty($pais_err) && empty($ciudad_err) && empty($edo_err) && empty($calle_err) && empty($cruza_err) && empty($num_err) && empty($tipo_err)){
        ...
        $sql = "UPDATE cliente SET name=?, ape1=?, ape2=?, correo=?, cel1=?, cel2=?, tel=?, contra=?, pais=?, ciudad=?, edo=?, calle=?, cruza=?, num=? WHERE idc=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssssssi",$param_name, $param_ape1,$param_ape2,$param_correo,$param_cel1,$param_cel2,$param_tel,$param_contra,$param_pais,$param_ciudad,$param_edo,$param_calle,$param_cruza,$param_num,$param_idc);
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p>Edite los valores de entrada y envíe para actualizar sus datos.</p>
